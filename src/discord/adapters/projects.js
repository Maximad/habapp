...
async function handleProjectCreate(interaction) {
  ...
  if (pipelineRaw && !pipeline) {
    return interaction.reply({ content: '❌ خط الأنابيب (pipeline) غير معروف.', ephemeral: true });
  }
  ...
  return interaction.editReply(
    `✅ تم إنشاء المشروع **${projName}** برمز **${slug}**.\n` +
      `المسار: ${pipeline ? `${pipeline.name_ar} (${pipeline.key})` : 'بدون مسار محدد'}\n` +
      `الوحدات: ${units.map(u => unitToArabic(u) || u).join(', ')}\n` +
...
async function handleProjectScaffold(interaction) {
  ...
  try {
    ...
    return interaction.reply({ content: '❌ المشروع غير موجود.', ephemeral: true });
  }
  ...
  if (effectivePipelineKey && !resolvedPipeline) {
    return interaction.editReply('❌ خط الأنابيب (pipeline) غير معروف.');
  }

  const created = await createTasksFromTemplates({ projectSlug: slug, pipelineKey: effectivePipelineKey });
  if (!created || created.length === 0) {
    return interaction.editReply('⚠️ لا توجد قوالب مهام لهذا النوع من المشاريع بعد.');
  }
  ...
  const pipelineCode = resolvedPipeline?.key || effectivePipelineKey || '---';
  return interaction.editReply(
    `✅ تم إنشاء ${created.length} مهمة تلقائياً لمسار ${pipelineLabel} (${pipelineCode}).`
  );
}
