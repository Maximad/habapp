...
- Implement production templates (A/B/C, internal vs external) in `src/core/templates.production.js`.
- Add status / loyalty system (قيادة، نواة، نشط، جاهزية، الدائرة المؤسسة...) in `src/core/status.js`.
...
- Project and task flows الآن تمر عبر خدمات النواة (`src/core/services`) مع طبقة محوّلات Discord (`src/discord/adapters`) مسؤولة عن التعامل مع discord.js.
...
- خدمة الحالة (`src/core/services/statusService.js`) تدعم أمر `/status` لعرض نظرة عامة أو حوافز النظام قبل ربط الأدوار فعلياً.
